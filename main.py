...
def ensure_user_record(uid: str, email: str):
    """Ensures a MongoDB user record exists WITHOUT overwriting intended roles."""
    now = datetime.utcnow()

    # Check if user already exists
    user = users_col.find_one({"uid": uid})
    if user:
        # Normalize quota_reset_date if stored as string
        reset = user.get("quota_reset_date")
        try:
            reset_dt = (
                reset
                if isinstance(reset, datetime)
                else (datetime.fromisoformat(reset) if isinstance(reset, str) else None)
            )
        except Exception:
            reset_dt = None
            ...
            users_col.update_one(
                {"uid": uid},
                {
                    "$set": {
                        ...
                        "quota_reset_date": (now + timedelta(days=1))
    ...
    # ------------------------------------------------------------------
    # NEW LOGIC:
    # If the record does NOT exist, create a safe minimal fallback user.
    # Role is NOT auto-assigned as admin anymore.
    # Backend /register endpoint must insert correct role BEFORE login.
    # ------------------------------------------------------------------

    fallback_role = "user"  # fallback only if backend didn't create a record
    ...
    print(f"➕ Created fallback MongoDB user record for {email} (role=user)")
...
async def register_user(payload: RegisterModel):
    ...
    if role == "manager" and passcode != MANAGER_PASSCODE:
    ...
    # Insert into MongoDB with correct role
